...
def DashboardView(page, tarea_controller):
    ...
    def refresh():
        ...
        for t in tarea_controller.obtener_lista(user.get("id_usuario", user.get("id"))):
    ...
    def add_task(e):
        success, msg = tarea_controller.guardar_nueva(user.get("id_usuario", user.get("id")), txt_titulo.value, "")
        if success:
            txt_titulo.value = ""
            refresh()
    return ft.View("/dashboard", [
        ft.AppBar(
            ...
            actions=[ft.IconButton(ft.Icons.EXIT_TO_APP, on_click=lambda e: page.session.store.clear() or page.go("/"))
            ]
        ),
        ft.column([
